...
############################### Setup Root Window + graphics Frame #########################################
def rootWindow():
    global root
    root = tk.Tk()
...
graphicsCanvas, graphicsFrame = graphicsWindow(root) #TODO testing
...
def textWindow(master): 
    ...
    text_title = tk.Text(textFrame, 
        ...
        padx=10, pady=10,
        relief=tk.RIDGE)
    text_title.insert(tk.INSERT, "Spring 1901 \n\n") #TODO update year
    text_title.pack()
...
textFrame, text_title, textSubframe, scrollbar, text = textWindow(root) #TODO testing
...
def buttonNext():
    ...
    button_results = tk.Button(textFrame, text="Next>>", command = lambda: var.set(1), bg="#4f5763", fg="white",font=("Courier", 12))
    button_results.pack()
...
# 3 -> Draws Move Outcomes and writes them to the text frame
def drawMoveOutcomes(Units, unitMoves):
    global text
...
def drawRetreats(Units, dislodgedList):    #TODO
    for Unit in Units:
        if Unit.status == "dislodged":
            Unit.drawUnitMove()
...
def phaseMoveDraw(turnMoves):#TODO NTH
    global text
    global all_units
    ...
    drawUnits(all_units)   
    buttonNext() # "NEXT" button input ->
    #Draw all unit orders on gameboard; list text commands to text frame
    drawMoves(all_units, turnMoves)
    buttonNext() # "NEXT" button input ->
    #Draw all order outcomes on gameboard; list outcomes on text frame
    drawMoveOutcomes(all_units, turnMoves)
    #Footer Bar
    text.insert(tk.INSERT, "\n ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ \n\n")

    #TODO -- save gamestate?
def phaseDislodgeDraw(turnMoves, dislodged_units):#TODO
    global text
    global all_units
    #Draw gameboard Background
    drawBackground(graphicsCanvas)
